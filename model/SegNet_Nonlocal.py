...
class SegNetNonlocal(nn.Module):
    def __init__(self, in_channels = 1, n_classes = 28):
        super().__init__()
        self.non_locals = nn.ModuleList([])
        # self.non_locals = nn.Sequential()
        for chn in [64,128,256,512, 512]:
            non_lb_blk = NONLocalBlock2D(in_channels=chn, sub_sample=False)
            self.non_locals.append(non_lb_blk)
