from model.stargan.model import Generator, Discriminator
...
import time
import datetime
from model.stargan.logger import Logger
...
class Solver(object):
    ...
    def restore_model(self, resume_iters):
        ...
        old_state_dict = self.D.state_dict()
        new_state_dict = {}
        for k, v in d_state_dict.items():
            ...
            if v.shape != old_state_dict[k].shape:
                continue
            new_state_dict[k] = v
        self.D.load_state_dict(new_state_dict, strict=False)
    ...
    def train(self):
        ...
        # for i in range(start_iters, self.num_epochs):
            ...
            if (i+1) % self.log_step == 0:
                ...
                if self.use_tensorboard:
                    ...
                    self.logger.scalar_summary(loss, i+1)
            ...
            if (i+1) % self.sample_step == 0 :
                with torch.no_grad():
                    ...
                    x_concat = torch.cat(x_fake_list, dim=3)#.transpose(2,-1)
                    x_concat = x_concat.flip(1)
                    # print('cat', x_concat.shape)
