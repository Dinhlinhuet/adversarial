...
from model.AgNet.core.blocks import M_Encoder
from model.AgNet.core.blocks import M_Conv
...
from model.AgNet.guided_filter_pytorch.guided_filter_attention import Upsample
...
class AG_Net(nn.Module):
    def __init__(self, n_classes, bn=True, BatchNorm=False):
        ...
        self.upsamples = [Upsample(256, 256, (10,10), stride=(8,8))
            , Upsample(128, 128, (6,6), stride=(4,4))
            , Upsample(64, 64, (3,3), stride=(2,2))
            , Upsample(32, 32, (3,3), stride=(1,1))]
    ...
    def forward(self, x):
        ...
        N, C, H, W = FG.size()
        FG_small = F.upsample(FG, size=(H//2, W//2), mode='bilinear')
        out = self.gf(FG_small, up5, FG,self.attentionblock6(FG_small,up5), 1)
        up6 = self.up6(out)

        FG = torch.cat([self.conv2(x_2), conv2], dim=1)
        N, C, H, W = FG.size()
        FG_small = F.upsample(FG, size=(H//2, W//2), mode='bilinear')
        out = self.gf(FG_small, up6, FG,self.attentionblock7(FG_small,up6),2)
        up7 = self.up7(out)

        FG = torch.cat([conv1, conv1], dim=1)
        N, C, H, W = FG.size()
        FG_small = F.upsample(FG, size=(H//2, W//2), mode='bilinear')
        out = self.gf(FG_small, up7, FG,self.attentionblock8(FG_small,up7), 3)
        up8 = self.up8(out)
        ...
        side_5 = self.upsamples[0](up5, output_size=(img_shape, img_shape))
        side_6 = self.upsamples[1](up6, output_size=(img_shape, img_shape))
        side_7 = self.upsamples[2](up7, output_size=(img_shape, img_shape))
        side_8 = self.upsamples[3](up8, output_size=(img_shape, img_shape))
...
class GridAttentionBlock(nn.Module):
    ...
    def forward(self, x, g):
        ...
        # print('input', self.phi(g).size())
        # print('output', theta_x_size[2:])
        # phi_g = self.upsample(self.phi(g), output_size=theta_x_size[2:])
