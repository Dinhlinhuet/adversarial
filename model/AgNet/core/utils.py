...
def get_data(data_path, img_name, img_size=256, gpu=True,flag=False):
    ...
    for i in range(batch_size):
        img_path = os.path.join(data_path, 'imgs', img_name[i])
        label_path = os.path.join(data_path, 'gt', img_name[i][:-3]+'jpg')
...
def dice_loss(pred, encoded_target):
    ...
    loss = loss_per_channel.sum() / pred.size(1)
    del pred, encoded_target

    return loss.mean()

def calculate_Accuracy(confusion):
    confusion=np.asarray(confusion)
    # print('confu', confusion)
    ...
    dice = np.sum(tp) / np.sum(confusion)
