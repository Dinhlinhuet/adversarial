...
def cw(args, idx, model, image, ground_truth, adv_target, num_iterations=20, gamma=0.07, no_background=True,
    ...
    w = Variable(torch.atan(image.clone()*2-1), requires_grad=True)
    ...
    for step in pbar:
        ...
        ms_ssim_val = ssim(img, image, data_range=1, size_average=False)
        ...
        #         print("Condition is", condition.sum())
    img = 1 / 2 * (nn.Tanh()(w) + 1)
    return img, logits, noise_total, noise_iteration, prediction_iteration, image_iteration
